...
const ChatBox: React.FC<ChatBoxProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      ref={chatBoxRef}
      className={`fixed bottom-16 sm:bottom-24 right-3 sm:right-6 w-[95vw] sm:w-full sm:max-w-md transition-all duration-300 ease-in-out transform ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 pointer-events-none'} ${isMinimized ? 'h-14 sm:h-16' : 'h-[80vh] sm:h-[450px]'} z-20`}
    >
      <div className="flex flex-col h-full border border-gray-200 rounded-xl overflow-hidden shadow-lg bg-white">
      <div className="bg-primary-600 text-white px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
